...
def load_mnist_v2(path=os.path.join(BASE_PATH, 'mnist')):
    """
    ...
    """
    train_dataset = MNIST(path, train=True, download=True, transform=transforms.ToTensor())
    test_dataset = MNIST(path, train=False, download=True, transform=transforms.ToTensor())
